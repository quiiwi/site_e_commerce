...
function internauteEstConnecteEtAdmin(){
	if (internauteEstConnecte() && $_SESSION['membre']['statut'] == 1) {
		//si membre connecté ET que son statut dans la session vaut 1, il est admin connecté
		return true;
	}else {
		return false;
	}
...
//-------------------- fonction de requête ----------------------
function executeRequete($req,$param = array()){ // cette fonction attend 2 valeurs : 1 requête SQL (obligatoire) et un array qui associe les marqueurs aux valeurs (non obligatoire car on lui a affecté au paramètre $param un array() vide par défaut)
	...
	return $result; // on retourne le sésultat de la requête de SELECT
